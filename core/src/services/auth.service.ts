...
export async function register(data: RegisterDto): Promise<void> {
  ...
  if (existingUser) {
    return throwError(400, 'User with given email already exists', 'REGISTER');
  }
...
export async function verifyOtp(data: VerifyOtpDto): Promise<AuthResponse> {
  ...
  if (!onboardingRecord) {
    return throwError(401, 'Invalid OTP or email', 'VERIFY-OTP', "Could not find onboarding record with given email and OTP");
  }

  if (new Date() > onboardingRecord.expiryAt) {
    return throwError(401, 'OTP has expired', 'VERIFY-OTP', "OTP has expired for given email");
  }
...
export async function resendOtp(email: string): Promise<void> {
  logger.debug({ context: 'RESEND-OTP', email }, 'Processing resend OTP for email');
  ...
  if (!record) {
    throwError(404, 'No pending registration found for this email', 'RESEND-OTP', "Could not find onboarding record for given email");
  }
...
export async function login(email: string, password?: string): Promise<AuthResponse> {
  ...
  if (!user) {
    return throwError(401, 'Invalid email or password', 'LOGIN', "User not found with given email");
  }

  if (password) {
    const isValid = await verifyPassword(user.password, password);
    if (!isValid) {
      return throwError(401, 'Invalid email or password', 'LOGIN', "Password verification failed for given email");
    }
  }
  ...
  logger.debug({ context: 'REFRESH' }, 'Processing token refresh');
    ...
    if (!user || user.refreshToken !== refreshToken) {
      ...
      if (user) {
           ...
           return throwError(401, 'Invalid refresh token', 'REFRESH', "Refresh token from cookie did not match db refresh token");
      } else {
        return throwError(401, 'Invalid refresh token', 'REFRESH', "User not found given userId in token payload");
      }
    }
...
export async function getSession(userId: string): Promise<Partial<User>> {
  ...
  if (!user) {
    return throwError(404, 'User not found', 'SESSION', `No user found with givenuserId`);
  }
  return {
    id: user.id,
    email: user.email,
